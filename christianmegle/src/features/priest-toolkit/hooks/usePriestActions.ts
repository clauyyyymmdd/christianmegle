...
import type { PriestActionMessage } from '../../../../shared/types/messages';
...
export function usePriestActions(signaling: SignalingClient) {
  ...
  const handleIncoming = (msg: PriestActionMessage) => {
    switch (msg.type) {
      case 'priest-penance':
      ...
      case 'priest-inscribe':
        setBookEntries((prev) => [
          ...prev,
          { id: crypto.randomUUID(), text: msg.text, timestamp: Date.now() },
        ]);
        break;
